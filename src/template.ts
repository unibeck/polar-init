...
const resolveAppDirectory = () => {
    ...
    throw new Error('App directory not found. Expected either "app" or "src/app" directory.');
}

const copyTemplate = async (templatePath: string, targetPath: string = resolveAppDirectory(), isFile: boolean = false) => {
    const cliDirectory = import.meta.dirname

    const templateDirectory = path.join(cliDirectory, "templates", templatePath)

    if (isFile) {
        fs.copyFileSync(path.join(templateDirectory), path.join(targetPath, templatePath))
    } else {
